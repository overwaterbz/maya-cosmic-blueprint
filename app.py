...
try:
    ...
    try:
        ...
        if not anthropic_client:
            raise Exception("Anthropic client initialization failed")
...
async def generate_enhanced_blueprint_analysis(user = Depends(get_current_user)):
    ...
    try:
        ...
        # Generate comprehensive AI analysis using OpenAI with error handling
        try:
            openai_client = openai.OpenAI(api_key=os.environ.get("OPENAI_API_KEY"))
            if not openai_client:
                raise Exception("OpenAI client initialization failed")
        except Exception as e:
            return {"error": "AI service temporarily unavailable", "fallback_analysis": f"Sacred {first_name}, your cosmic blueprint holds infinite wisdom. Your spiritual journey is uniquely yours, guided by ancient Maya wisdom and cosmic intelligence. The Magic is You!"}
...
async def generate_enhanced_soul_contract(user = Depends(get_current_user)):
    ...
    try:
        ...
        # Generate comprehensive soul contract using OpenAI with error handling
        try:
            openai_client = openai.OpenAI(api_key=os.environ.get("OPENAI_API_KEY"))
            if not openai_client:
                raise Exception("OpenAI client initialization failed")
        except Exception as e:
            return {"error": "AI service temporarily unavailable", "fallback_contract": f"Sacred {first_name}, your soul contract is written in the stars. Your spiritual journey is uniquely yours, guided by ancient Maya wisdom. Trust your cosmic blueprint and the magic within you!"}
...
Make it authentic, powerful, and deeply personal."""
        ...
        try:
            ...
            <h5>3. Integration (3 minutes)</h5>
            <p>Place hands on heart and repeat: "I am aligned with my {day_sign} power and {element} wisdom."</p>
...
Make it authentic, practical, and deeply meaningful."""
        ...
        try:
            ...
            <p>Journal about your day's {galactic_tone} experiences and thank your guides.</p>
...
if __name__ == "__main__":
    import uvicorn
    # Configure proper port for Cloud Run deployment
